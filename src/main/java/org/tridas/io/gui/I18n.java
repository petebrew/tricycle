 ...
 * <p>
 * Java's ResourceBundles are intended to do this, but they don't provide the level of support that
 ...
 * </pre>
 * 
 * Of course, you should internationalize this. But you want to change the text, the mnemonic, and
 * the keyboard accelerator. If you only had ResourceBundle to work with, you might end up with 3
 * lines of translation for every word in your program. Plus, you'd have to know the name of the
 * I18n file you used, each time you wanted to use it. Most other libraries (like Powerplant on Mac
 * and Win95 resources) let you put them all in one line, which makes the translator's job much
 * easier. That's what this class does.
 * </p>
 * <p>
 ...
 * </p>
 * <ul>
 * <li>"copy" is the <i>key</i>. When you want to refer to this entry, you'll ask for it by this
 * name. The user never sees this.
 * <li>"Copy" (without the &amp;) is the <i>text</i> of this key. This is what users will see in
 * menuitems, buttons, and so forth.
 * <li>"C" (the thing right after the &amp;) is the <i>mnemonic</i>. On most platforms (all except
 * Mac), this letter is underlined, and users can jump to it by pressing this letter, or Alt and
 * this letter (depending on platform and context).
 * <li>"accel C" is the <i>keystroke</i>. The user can press this at any time to invoke this
 * command. You can use the modifiers "shift", "control", "alt", and "meta", or the special modifier
 * "accel". "accel" is automatically turned into "control" on PCs (Windows and Linux), and "meta"
 * (Java's term for "command") on Macs. You should normally use the generic "accel" modifier. You
 * can also combine modifiers, like "shift accel S" (the standard keystroke for "Save As..."). The
 * class javax.swing.KeyStroke takes these strings as input: you can say
 * 
 * <pre>
 ...
 * keystrokes and mnemonics. You can simply use the Builder factory to do the dirty work for you:
 * 
 * <pre>
 * JMenuItem copy = Builder.makeMenuItem(&quot;copy&quot;);
 * </pre>
 * 
 * Of course, if you just want the text (for making a printout, for example), you should still use
 * I18n.getText().
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * These methods get their values from the resource bundle called "TextBundle". That is, the file is
 * called "TextBundle.properties", or some variant, like "TextBundle_de_DE.properties".
 * </p>
 * <h2>Left to do</h2>
 * <ul>
 * <li>The way getMnemonic() is set up, "Save &as..." will still show it as "S&ave as...". Bad, but
 * not fatal.
 ...
 * <li>in getKeyStroke(), why not use Toolkit.getDefaultToolkit().getMenuShortcutKeyMask() instead
 * of (?:) for command/control choice?
 * </ul>
 * 
 * @see java.util.ResourceBundle
 * @author Ken Harris &lt;kbh7 <i style="color: gray">at</i> cornell <i style="color: gray">dot</i>
 *         edu&gt;
...
public class I18n {
	 ...
	 * <p>
	 * For example, if the localization file has the line <code>copy = &amp;Copy [accel C]</code>,
	 ...
	 * <p>
	 * For example, if the localization file has the line <code>copy = &amp;Copy [accel C]</code>,
	 * the string "control C" is returned (or on the Mac, "meta C").
	 * </p>
	 * <p>
